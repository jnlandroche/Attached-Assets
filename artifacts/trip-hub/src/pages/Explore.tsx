...
import { MapPin, Navigation, Heart, ExternalLink, Globe } from "lucide-react";
          ...
          <TabsList className="w-full h-auto flex flex-wrap gap-2 bg-transparent p-0 justify-start">
            {categories.map(cat => (
              <TabsTrigger 
                key={cat.id} 
                value={cat.id}
                className="rounded-full px-4 py-2 bg-sand-100 text-ink-700 data-[state=active]:bg-ink-950 data-[state=active]:text-white shadow-none data-[state=active]:shadow-card transition-all"
              >
                {cat.label}
              </TabsTrigger>
            ))}
            ...
            <TabsContent key={cat.id} value={cat.id} className="m-0 space-y-4 outline-none">
              {isLoading ? (
                ...
                <div className="py-12 text-center text-ink-900/40">
                  <p>No entries found for {cat.label.toLowerCase()} yet.</p>
                </div>
              ) : (
                currentEntries.map(entry => (
                  <div key={entry.id} className="bg-white rounded-3xl shadow-card overflow-hidden relative">
                    ...
                    <div className="p-5">
                      ...
                      <div className="flex flex-wrap gap-3 text-xs font-bold text-ink-800 mb-4">
                        {entry.priceTier && (
